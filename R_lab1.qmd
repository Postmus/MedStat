...
The `summarise()` function creates a new data frame containing the summary statistics you specify. The functions `mean()`, `sd()`, `median()`, and `IQR()` are built-in base R functions that work within `summarise()`. The `na.rm = TRUE` argument ensures that any missing values are ignored in the calculations.

Let's break down what each summary statistic represents:

- **Mean**: The average age of mothers in the dataset.
- **Standard Deviation (sd)**: A measure of the spread or variability of ages around the mean.
- **Median**: The middle value when all ages are arranged in order.
...
Calculate the mean, standard deviation, median, and IQR for the variable `lwt` using the `summarise()` function. Additionally, create a histogram to determine the shape of its distribution and decide which summary measures are most appropriate to report.
:::
