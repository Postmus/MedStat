...
Manually calculate the corresponding 95% confidence interval.
:::

You can also have SPSS calculate this 95% confidence interval. To do this, go to `Analyze => Descriptive Statistics => Explore`. Click on 'Statistics.' Here you can specify which confidence interval you want to calculate. By default, it is set to 95%, so there is no need to change anything. Click 'Cancel,' and then select 'Statistics' under 'Display.' The output will then contain only the desired descriptive statistics. Press 'OK' to generate this output.

::: {.callout-important icon=false title="Question 3"}
How does the 95% confidence interval given by SPSS compare to the 95% confidence interval that you manually computed?
:::
