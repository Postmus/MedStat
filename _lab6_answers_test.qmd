...
hardheid <- read_sav("datasets/hardheid Engels.sav")
...
```{r}
model_ancova <- lm(pocketdepth ~ I(age - 45) + alcohol, data = pockets)
summary(model_ancova)

model_hardheid_full <- lm(hardness ~ composite_type * curing_time, data = hardheid)
summary(model_hardheid_full)
Anova(model_hardheid_full, type="III")
